...
const EditVariableForm = (props) => {
  ...
  const [type, setType] = useState(0)
   ...
   async function getVariable() {
    ...
    try
    {
      let response = await API.variables.list(props.selectedVariable[0]);
  ...
  useEffect(() => {
      getVariable(props.selectedVariable[0])
  ...
  function handleSubmit(e) {
    ...
    async function postData(params) {
      try 
      {
        const response = await API.variables.edit(props.selectedVariable[0], params);
  ...
  return (
    <>
            ...
            <input name="name" value={name} type='text' placeholder='name' required='required' id="name" onChange={e => setName(e.target.value)} ></input>
            </div>
            ...
            <input name="minion" value={value} type={encrypted ? ('password') : 'text'} placeholder='value' id='Minion' onChange={e => setValue(e.target.value)}></input>
            </div>
            <div className='inputRow'>
            <label className='label'>Variable Type:</label>
            <select name="minion" value={type} type='text' placeholder='variableType' required='required' id='Minion' onChange={e => setType(e.target.value)}>
            ...
            <div className="inputRow">
            <p></p>
            <Button type='submit' className="saveButton" variant="contained" color="secondary">Edit Variable</Button>
            </div>
            </form>
