...
const EditPlaybookForm = (props) => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();
  let playbook = []

  let variableSetDefaults = []

  let functionDefaults = []
  ...
  function filterPlaybook () {
    if (props.selectedPlaybook != null)
  {
  ...
  function setDefaults () {
    variableSetDefaults.length = 0;
    functionDefaults.length = 0;
    if(playbook.length > 0){
      props.variableSetData.map((variableSet, index) =>
      {
          if (playbook[0].variable_sets.includes(variableSet.id))
          {
            variableSetDefaults.push({value: variableSet.id, label: variableSet.name})
          }  
      });
      ...
      { 
        ...
        {
            if (playbook[0].functions.includes(functionName.id))
            {
              functionDefaults.push({ value: functionName.id, label: `${module.name}.${functionName.name}` })
            }
        })
      });
    }
  ...
  useEffect(() => 
    {
        ...
        setDefaults()
        if(playbook.length > 0){
          setName(playbook[0].name)
          setMinion(playbook[0].minion)
          setSchedule(playbook[0].schedule)  
        }
        setVariableSets(variableSetDefaults)
        setFunctions(functionDefaults)
  ...
  props.functionData.map((module, index) =>
  {
    ...
    const [name, setName] = useState(playbook.length > 0 ? playbook[0].name : '');
    const [minion, setMinion] = useState(playbook.length > 0 ? playbook[0].minion.id : '');
    const [schedule, setSchedule] = useState(playbook.length > 0 ? playbook[0].schedule : '');
    const [functions, setFunctions] = useState(variableSetDefaults)
    const [variableSets, setVariableSets] = useState([functionDefaults])
...
function handleSubmit(e) {
  ...
  async function postData(params) {
    try 
    {
      const response = await API.playbooks.edit(playbook[0].id, params);;
  ...
  return (
    <>
            ...
            <label className='label' for='Minion'>Variable Set:*</label>
                <Select options={variableSetData} value={variableSets} placeholder="Select Variable Set" isMulti autoFocus isSearchable onChange={setVariableSets}/>
            </div>
            ...
            <label className='label' for='Minion'>Function:*</label>
                <Select options={functionData} value={functions} placeholder="Select Function" isMulti autoFocus isSearchable onChange={setFunctions}/>
            </div>
...
export default EditPlaybookForm;